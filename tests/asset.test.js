...
describe ('Asset API Master Test Suite', () => {
  ...
  describe('Asset Lifecycle API Tests', () => {
      ...
      /**
       * TC-003: Read Asset Inventory (GET)
       * Goal: Verify the system returns the correct list of tools.
       */
      test('TC-003: Should retrieve the list of assets', async () => {
          const res = await request(app)
              .get('/api/assets')
              .set('x-auth-token', testToken);
      ...
      /**
       * TC-004: Update Asset Details (PUT)
       * Goal: Verify that editing an asset reflects changes in the database.
       */
      test('TC-004: Should update asset details (dailyRate and status)', async () => {
          const updates = {
              dailyRate: 55.00,
              status: 'Maintenance'
          };
  ...
  describe('Maintenance State Transitions', () => {
      test('TC-005: Should successfully clear maintenance and update inspection date', async () => {
      ...
      test('TC-006: Should fail to clear maintenance if asset is already Available', async () => {
 ...
 */
  describe('Conditional Deletion Safety', () => {
      test('TC-007: Should block deletion of a Rented asset', async () => {
      ...
      test('TC-008: Should allow deletion of an Available asset', async () => {
 ...
 */
  describe('API Security & Authorization', () => {
      ...
      test('TC-009: Should reject GET /api/assets if no token is provided', async () => {
          const res = await request(app)
      ...
      test('TC-010: Should reject POST /api/assets if token is malformed/invalid', async () => {
          const res = await request(app)
              .post('/api/assets')
